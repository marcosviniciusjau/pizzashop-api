...
var orders = (0, import_pg_core2.pgTable)("orders", {
  ...
  customerEmail: (0, import_pg_core2.text)("customer_email").references(() => users.email, {
    onDelete: "set null"
  }).notNull(),
...
var restaurants = (0, import_pg_core4.pgTable)("restaurants", {
  id: (0, import_pg_core4.text)("id").$defaultFn(() => (0, import_nanoid4.nanoid)()).primaryKey(),
...
var evaluations = (0, import_pg_core5.pgTable)("evaluations", {
  id: (0, import_pg_core5.text)("id").$defaultFn(() => (0, import_nanoid5.nanoid)()).primaryKey(),
  customerName: (0, import_pg_core5.text)("customer_id").references(() => users.name),
  restaurantId: (0, import_pg_core5.text)("restaurant_id").references(() => users.name),
...
var authLinks = (0, import_pg_core6.pgTable)("auth_links", {
  id: (0, import_pg_core6.text)("id").$defaultFn(() => (0, import_nanoid6.nanoid)()).primaryKey(),
...
var products = (0, import_pg_core7.pgTable)("products", {
  id: (0, import_pg_core7.text)("id").$defaultFn(() => (0, import_nanoid7.nanoid)()).primaryKey(),
...
var sendAuthenticationLink = new import_elysia3.default().post(
  ...
  async ({ body }) => {
    ...
    const userFromEmail = await db.query.users.findFirst({
      where(fields, { eq: eq17 }) {
        return eq17(fields.email, email);
      }
    });
    if (!userFromEmail) {
      throw new UnauthorizedError();
    }
    const authLinkCode = (0, import_nanoid8.nanoid)();
...
var createOrder = new import_elysia5.default().use(authentication).post(
  ...
  async ({ params, body, set }) => {
    ...
    const products2 = await db.query.products.findMany({
      where(fields, { eq: eq17, and: and11, inArray: inArray2 }) {
        return and11(
          eq17(fields.restaurantId, restaurantId),
    ...
    const customerExists = await db.query.users.findFirst({
      where: (users2, { eq: eq17 }) => eq17(users2.email, customerEmail)
    });
    if (!customerExists) {
...
var approveOrder = new import_elysia6.default().use(authentication).patch(
  ...
  async ({ getManagedRestaurantId, set, params }) => {
    ...
    const order = await db.query.orders.findFirst({
      where(fields, { eq: eq17, and: and11 }) {
        return and11(
          eq17(fields.id, orderId),
          eq17(fields.restaurantId, restaurantId)
        );
      }
    });
...
var getOrders = new import_elysia8.default().use(authentication).get(
  ...
  async ({ query, getCurrentUser, set }) => {
    ...
    }).from(orders).innerJoin(users, (0, import_drizzle_orm9.eq)(users.name, orders.customerName)).where(
...
var authenticateFromLink = new import_elysia14.default().use(authentication).get(
  ...
  async ({ signUser, query, set }) => {
    ...
    const managedRestaurant = await db.query.restaurants.findFirst({
      where(fields, { eq: eq17 }) {
        return eq17(fields.managerId, authLinkFromCode.userId);
      }
    });
    await signUser({
...
var getManagedRestaurant = new import_elysia15.default().use(authentication).get("/managed-restaurant", async ({ getManagedRestaurantId }) => {
  ...
  const restaurant = await db.query.restaurants.findFirst({
    where(fields, { eq: eq17 }) {
      return eq17(fields.id, restaurantId);
    }
  });
  if (!restaurant) {
...
var getProducts = new import_elysia26.default().use(authentication).get("/get-products", async ({ getManagedRestaurantId }) => {
  ...
  try {
    ...
    }).from(orderItems).leftJoin(orders, (0, import_drizzle_orm21.eq)(orders.id, orderItems.orderId)).leftJoin(products, (0, import_drizzle_orm21.eq)(products.id, orderItems.productId)).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(orders.restaurantId, restaurantId))).groupBy(products.name, products.id);
    return product;
  } catch (err) {
    console.log(err);
  }
...
var getCustomers = new import_elysia27.default().use(authentication).get("/get-customers", async ({ getManagedRestaurantId }) => {
  ...
  try {
    const customer = await db.select({
      ...
      name: users.name,
      phone: users.phone
    }).from(users).where((0, import_drizzle_orm22.and)((0, import_drizzle_orm22.eq)(users.role, role)));
    return customer;
  } catch (err) {
    console.log(err);
  }
